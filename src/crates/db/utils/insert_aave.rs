...
use crate::helpers::{
    math::{
        ...
        fetch_reserve_snapshots_aave_v3, fetch_reserve_snapshots_aave_v4, ray_to_bps,
        u256_to_bigdecimal,
    },
    vectors::{vec_addr_to_string, vec_assets_v4_underlying_to_string},
};
use crate::parser::aave::data::abi::{
    AAVEv1Pool::AAVEv1PoolInstance, AAVEv2Pool::AAVEv2PoolInstance, AAVEv3Pool::AAVEv3PoolInstance,
    AAVEv4CoreHub::AAVEv4CoreHubInstance,
};
...
pub async fn insert_aave_v2(
    ...
    let snapshots =
        fetch_reserve_snapshots_aave_v2(&parser, provider.clone(), &reserves_list).await;
...
pub async fn insert_aave_v3(
    ...
    let snapshots =
        fetch_reserve_snapshots_aave_v3(&parser, provider.clone(), &reserves_list).await;
...
pub async fn insert_aave_v4(
    conn: &DBEngine,
    chain_id: i64,
    parser: AAVEv4CoreHubInstance<DynProvider>,
    provider: &DynProvider,
    assets: &Vec<AssetV4>,
    reserves_list: &Vec<Address>,
) {
    let protocol = &conn
        .insert_protocols(
            ...
            "AAVE-V4",
            "Lending",
            Some("src/crates/parser/aave/data/abi_aave_v4.json"),
        )
        .await
        .expect("Failed to insert protocol");
    ...
    let reserves_v4_csv: String = vec_addr_to_string(&reserves_list).await;

    let snapshots = fetch_reserve_snapshots_aave_v4(&parser, &assets).await;

    let markets = &conn
        .insert_markets(
            protocol.id,
            chain_id,
            &AAVE_V4_CORE_HUB.to_string(),
            "lending",
            &reserves_v4_csv,
        )
        .await
